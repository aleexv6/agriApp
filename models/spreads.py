...
def getSpread(dfFutures, ticker, firstLegMonth, secondLegMonth, dataYears, expiYearMax):
    ...
    merged = merged[['Date', 'Spread', 'Expiration_x', 'Expiration_y', 'Expiration Full Date_x', 'Expiration Year_x']]


    dfList = []
    years = df['Expiration Year'].unique()[2:]
    for year in years - 1:
        data = merged[(merged['Expiration_x'] == f'{firstLegMonth}{pad_number(year + 1)}') & (merged['Expiration_y'] == f'{secondLegMonth}{pad_number(year + 1)}')]
        if not data.empty:
    ...
    for expiYear in dfSpread['Expiration Year_x'].unique():
        ...
        spreadDict.append({'spreadYear': f"{firstLegMonth}{expiYear} - {secondLegMonth}{expiYear}", 'spreadData': tmp[['Date','Spread']].to_dict()})
    return spreadDict